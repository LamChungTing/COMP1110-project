...
Now with VIP priority: VIPs go to the front of their size queue.
"""

class Customer:
    """Represents a customer party with their arrival, dining preferences, and VIP status."""
    
    def __init__(self, party_size, arrival_time_str, dining_time_minutes, max_waiting_time_minutes, vip):
        ...
        self.vip = int(vip)   # 1 = VIP, 0 = Standard
    ...
    try:
        with open(file_path, 'r') as file:
            ...
            for line in file:
                ...
                # New format: 5 columns (VIP column added)
                if len(data) == 5:
                    new_customer = Customer(data[0], data[1], data[2], data[3], data[4])
...
def RunSimpleSimulation(customers, table_counts):
    ...
    # Helper to count VIPs in a queue (to find insertion position)
    def count_vips(queue):
        return sum(1 for cust in queue if cust.vip == 1)
    
    # Helper to add a customer to the correct queue with VIP priority
    def enqueue_customer(cust):
        # Choose the correct queue based on party size
        ...
        # VIP logic: VIPs jump to the front, after existing VIPs
        if cust.vip == 1:
            # Count how many VIPs are already in this queue
            vip_count = count_vips(q)
            # Insert at position = vip_count (so all VIPs stay before standards)
            q.insert(vip_count, cust)
        else:
            # Standard customers go to the end
            q.append(cust)
    ...
    while (customer_index < len(customers) or 
        ...
        while (customer_index < len(customers) and customers[customer_index].arrival_time == current_time):
            ...
            vip_label = " (VIP)" if cust.vip == 1 else ""
            simulation_logs.append(f"Time {format_time(current_time)}: Party of {cust.party_size}{vip_label} arrived. Queue sizes: S:{len(queue_small)} M:{len(queue_medium)} L:{len(queue_large)}")
            customer_index += 1
...
# ============ MAIN EXECUTION ============
customers = OpenCustomerData("queueVIP.txt")
